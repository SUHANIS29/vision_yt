...
import calendar
from textblob import TextBlob #comment sentiment analysis
from collections import Counter #	Counts keywords or categories for ranking and plotting
...
if st.button("Analyze Channels"):
    if not url_list:
        ...
        if all_data:
            ...
            # )
            upload_freq["Upload Frequency"] = upload_freq["Videos Per Day"].apply(
                lambda x: "Daily+" if x >= 1 else 
                        "Every 2-3 days" if x >= 0.33 else 
                        "Weekly" if x >= 0.14 else 
                        "Monthly" if x >= 0.03 else "Irregular"
            )
            ...
            for channel_name in final_df["Channel"].unique():
                ...
                # Better approach:
                def calculate_consistency_score(avg_gap):
                    if pd.isna(avg_gap):
                        return 0
                    
                    # Ideal gap for different patterns
                    if avg_gap <= 1:
                        return max(90, 100 - (avg_gap * 10))  # Daily: 90-100%
                    elif avg_gap <= 3:
                        return max(70, 90 - ((avg_gap - 1) * 10))  # Every 2-3 days: 70-90%
                    elif avg_gap <= 7:
                        return max(50, 70 - ((avg_gap - 3) * 5))  # Weekly: 50-70%
                    elif avg_gap <= 30:
                        return max(20, 50 - ((avg_gap - 7) * 1.3))  # Monthly: 20-50%
                    else:
                        return min(20, 100 / avg_gap)  # Irregular: <20%

                consistency_score = calculate_consistency_score(avg_gap)
                ...
                with col3:
                    # Use avg_gap for consistency instead of upload_freq table
                    if not pd.isna(avg_gap):
                        if avg_gap <= 1:
                            freq_category = "Daily+"
                        elif avg_gap <= 3:
                            freq_category = "Every 2-3 days"  
                        elif avg_gap <= 7:
                            freq_category = "Weekly"
                        elif avg_gap <= 30:
                            freq_category = "Monthly"
                        else:
                            freq_category = "Irregular"
                    else:
                        freq_category = "N/A"
