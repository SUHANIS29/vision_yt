...
# NEW: ENHANCED CHANNEL COMPARISON VISUALIZATIONS
            if len(final_df["Channel"].unique()) > 1:
                ...
                # st.markdown("### 👀 Average Views Comparison")
                ...
                fig_avg_views.update_layout(showlegend=False)
                # st.plotly_chart(fig_avg_views, use_container_width=True)
                
                # 3. Total Views Comparison
                # st.markdown("### 🎯 Total Views Comparison")
                fig_total_views = px.bar(
                    channel_comparison, 
                    x="Channel", 
                    y="Total Views",
                    color="Total Views",
                    # title="Total Channel Views Comparison",
                    text_auto='.2s',
                    color_continuous_scale="Reds"
                )
                fig_total_views.update_layout(showlegend=False)
                # st.plotly_chart(fig_total_views, use_container_width=True)

 # 4. Combined Performance Dashboard
            # st.markdown("### 🏆 Multi-Metric Performance Dashboard")
            ...
            def get_fastest_growing_category(channel_df):
                ...
                if len(category_growth) > 0:
                    fastest_growing = category_growth.loc[category_growth["Avg Views"].idxmax()]
                    return fastest_growing.name, category_growth.to_dict()
                else:
                    return "Not enough data", {}
            ...
            for channel_name in final_df["Channel"].unique():
                ...
                with time_col2:
                    ...
                    if len(day_perf) > 1:
                        # Fixed: Ensure proper data format for day performance
            ...
            if len(final_df["Channel"].unique()) > 1:
                ...
                with col1:
                    # Subscribers vs Avg Views scatter plot
            ...
            for channel in final_df["Channel"].unique():
                ...
                if all_comments:
                    ...
                    pos = round((counts["Positive"] / total) * 100, 2) if total else 0
                    neu = round((counts["Neutral"] / total) * 100, 2) if total else 0
                    neg = round((counts["Negative"] / total) * 100, 2) if total else 0
            ...
            if word_view_data:
                ...
                for _, row in comparison_df.iterrows():
                    ...
                    # Specific recommendations based on position
                    ...
                    if row["Content Strategy"] == "Focus more on Shorts":
                        st.markdown("- **Strategy:** Double down on Shorts content")
                    elif row["Content Strategy"] == "Long-form content performs better":
            ...
            # st.success("All research questions have been answered with data-driven insights. Use the individual channel recommendations to optimize your YouTube strategy for 2025!")